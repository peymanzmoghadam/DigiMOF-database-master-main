...
numeric = R('^\d{1,3}$')
comma = (W(',') | T(',')).hide()
characters_and_digits = R('^([a-zA-Z0-9])+')


numeric_delim = Optional(delim) + Optional(characters_and_digits) + Optional(delim) + Optional(characters_and_digits) + Optional(delim) + Optional(characters_and_digits) + Optional(delim) + Optional(delim) + Optional(characters_and_digits) + Optional(characters_and_digits) + Optional(delim) + Optional(characters_and_digits) + Optional(delim) + Optional(characters_and_digits) + Optional(delim) + Optional(characters_and_digits) + Optional(delim) + Optional(characters_and_digits) + Optional(characters_and_digits)
...
label_type = (Optional(I('reference') | I('comparative')) + R('^(material|structure|compound|MOF|ligand|chemical|dye|derivative|complex|example|intermediate|product|formulae?|preparation)s?$', re.I))('role').add_action(join) + Optional(colon).hide()
...
lenient_chemical_label = (numeric('label') + Optional(numeric_delim)) | R('^([A-Z]\d{1,3})$')('label') | strict_chemical_label
...
formula = (
    ...
    R('^(GABA|NO|\(\d\)H|KCl)$') |
    (R('(?![aA-zZ]+\-type(s?)$)(UBMOF|ZJU|SNU|MAF|MCF|Ir-MOF|JUC|FJI|UHM|MUV|BUT|ZJNU|Tb|she|CTH|pek|FDM|MODF|USF|NJFU|ZJU|CMOF|CTH|TPMOF|IZE|pbz|PNMOF|gea|MFM|UNLPF|PIZOF|soc|MOAAF|Y|Ho|CPO|JLU|aea|NOTT|NU|MMPF|UTSA|CPM|U[iI]O|MOF|IRMOF|T-MOF|NTU|MIL|HKUST|HNUST|LIC|PCN|ZIF|CPL|CALF|UMCM|DUT$)') + R('^[\-‐‑⁃‒–—―−－⁻]$') + R('^[a-zA-Z0-9]+$') + R('\(') + R('^Hf$') + R('\)')) |
    (R('^(?![aA-zZ]+\-type(s?)$)(UBMOF|ZJU|SNU|MAF|MCF|Ir-MOF|JUC|FJI|UHM|MUV|BUT|ZJNU|Tb|she|CTH|pek|FDM|MODF|USF|NJFU|ZJU|CMOF|CTH|TPMOF|IZE|pbz|PNMOF|gea|MFM|UNLPF|PIZOF|soc|MOAAF|Y|Ho|CPO|JLU|aea|NOTT|NU|MMPF|UTSA|CPM|U[iI]O|MOF|IRMOF|T-MOF|NTU|MIL|HKUST|HNUST|LIC|PCN|ZIF|CPL|CALF|UMCM|DUT$)') + R('^[\-‐‑⁃‒–—―−－⁻]$') + R('^[a-zA-Z0-9]+$')) |
    (R(u'^(?![aA-zZ]+\-type(s?)$)(UBMOF|ZJU|SNU|MAF|MCF|Ir-MOF|JUC|FJI|UHM|MUV|BUT|ZJNU|Tb|she|CTH|pek|FDM|MODF|USF|NJFU|ZJU|CMOF|CTH|TPMOF|IZE|pbz|PNMOF|gea|MFM|UNLPF|PIZOF|soc|MOAAF|Y|Ho|CPO|JLU|aea|NOTT|NU|MMPF|UTSA|CPM|U[iI]O|MOF|IRMOF|T-MOF|NTU|MIL|HKUST|HNUST|LIC|PCN|ZIF|CPL|CALF|UMCM|DUT)[:;\[\]\)\{\}\]{0,3}[\-‐‑⁃‒–—―−－⁻][A-Za-z0-9_-]+[:;\[\]\)\{\}\]{0,3}[A-Za-z0-9_-]*[:;\[\]\)\{\}\]{0,3}[A-Za-z0-9_-]*[:;\[\]\)\{\}\]{0,3}[A-Za-z0-9_-]*[:;\[\]\)\{\}\]{0,3}[A-Za-z0-9_-]+$', re.I)) |
    (R('^(UBMOF|ZJU|SNU|MAF|MCF|Ir-MOF|JUC|FJI|UHM|MUV|BUT|ZJNU|Tb|she|CTH|pek|FDM|MODF|USF|NJFU|ZJU|CMOF|CTH|TPMOF|IZE|pbz|PNMOF|gea|MFM|UNLPF|PIZOF|soc|MOAAF|Y|Ho|CPO|JLU|aea|NOTT|NU|UTSA|MMPF|CPM|U[iI]O|MOF|T-MOF|IRMOF|NTU|MIL|HKUST|HNUST|LIC|PCN|ZIF|CPL|CALF|UMCM|DUT)([\-‐‑⁃‒–—―−－⁻])([a-zA-Z0-9]+)$')) |
    R('^(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\dAs2O\d)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\dO\d\(OH\)\d\(RCO2\)\d\d?)(?![aA-zZ]+\-?\d?\d?\-ligands)(?!(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d\(CO2\)\d?\(?N?N?\)?)(?!^(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\($)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d?O?\(C?H?3?COO\)\d\s?\d?H?2?O?)(?!M1)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d\(μ(3|4)-OH?\)\d?\(R?CO2R?\)\]?\d?)(?!\[?Zn\d\(μ4-O\)\(O2CR?\)\d\])(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d?\((µ-OAc|µ-AcO|AcO|OAc)\)\d?\s?·?H2O)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d?\((µ-OAc|µ-AcO|AcO|OAc)\)\d?\s?\(H2O)(?!Me\dNH\d)(?!(\[?Zn\dO?\(O2C))(?!^M\(II\))(?!.*\(OOC\)\d$)(?!Co\(Tt\)\d?$)(?!\{?.*\(COO−?\)\d\}?$)(?!(M|Mn|Cu|Zn|Y|Co|Ni|Fe|Zr|Cd)\((II|i|ii|iii)\)$)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d?O?\(R?CO2R?\)\d\]?$)(?!(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\dO?\+?.?.?.?$)(?!UiO-type$)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\(NO\d?\)\d?\]?.?.?.?.?.?.?.?$)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\(ClO\d?\)\d?\]?.?.?.?.?.?.?.?$)(?!Cu\(NO\d?\).?.?.?.?.?.?.?.?$)(?!\[Zn3\(EBTC\)2\]∞$)(?!\{?Cu\d\(O2?CR\)\d\}?$)(?!HCl$)(?!Cu\(II\)$)(?!Zr6O8\(CH3COO\)26\+$)(?!Cu\(NO3\)2·3H2O$)(?!Zr6$)(?!Cu\(NO3\)2$)(\[?)(\{)?(\[)?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)(\s?)(L|\d|\(|\-)')
)
node_ion=R(u'^((Zn|Mn|Cu|Fe|Al|Cd|Zr|Co|Mg|V|Ti|Pb|Ni|Na|Sc|Cr|Y|Nb|Mo)|[Mm]olybdenum|[Nn]iobium|[Yy]ttrium|[Cc]hromium|[Ss]candium|[Ss]odium|[Nn]ickel|[Ll]ead|[Tt]itanium|[Zz]inc|[Mm]anganese|[Vv]anadium|[Mm]agnesium|[Cc]opper|[Ii]ron|[Ff]errous|[Aa]lumin(ium|um|o)|[Cc]admium|[Zz]irconium|[Cc]obalt)(\W|[0-9\+\-A-Z\.ivl\(\)\·\[\\]\\(\\)\\{\\}]+)?$')
metal_precursor=(node_ion+Optional(R(u'^[[Ii]so|[Pp]rop|[Oo]xide|oate|[Ss]ul(f|ph)|[Nn]itr|ate|ite|[Pp]er|[Oo]cta|[Tt]etra|ide|[Ii]od|[cC](h)lor|[Mm]angan|[Tt]ri|[Hh]ydrogen|[Cc]arbon|[Hh]ydr|[hH]ypo|[Dd]i|[Pp]hosp|[Ff]luor|[Ff]err|[Cc]arbamate]+?$')))
...
MOF_name = (R('^(?![aA-zZ]+\-type(s?)$)(UBMOF|ZJU|SNU|MAF|MCF|Ir-MOF|JUC|FJI|UHM|MUV|BUT|ZJNU|Tb|she|CTH|pek|FDM|MODF|USF|NJFU|ZJU|CMOF|CTH|TPMOF|IZE|pbz|PNMOF|gea|MFM|UNLPF|PIZOF|soc|MOAAF|Y|Ho|CPO|JLU|aea|NOTT|NU|MMPF|UTSA|CPM|U[iI]O|MOF|IRMOF|T-MOF|NTU|MIL|HKUST|HNUST|LIC|PCN|ZIF|CPL|CALF|UMCM|DUT$)') +R('^[\-‐‑⁃‒–—―−－⁻]$')+ R('^[a-zA-Z0-9]+$') + R('\(') + R('^[a-zA-Z0-9]{0,3}$') + R('\)')) | R('^(UBMOF|ZJU|SNU|MAF|MCF|Ir-MOF|JUC|FJI|UHM|MUV|BUT|ZJNU|Tb|she|CTH|pek|FDM|MODF|USF|NJFU|ZJU|CMOF|CTH|TPMOF|IZE|pbz|PNMOF|gea|MFM|UNLPF|PIZOF|soc|MOAAF|Y|Ho|CPO|JLU|aea|NOTT|NU|MMPF|UTSA|CPM|U[iI]O|MOF|IRMOF|T-MOF|NTU|MIL|HKUST|HNUST|LIC|PCN|ZIF|CPL|CALF|UMCM|DUT$)[,:;\.\[\]\(\)\{\}\]{0,3}[A-Za-z0-9_-]+[,:;\.\[\]\(\)\{\}\]{0,3}[A-Za-z0-9_-]*[,:;\.\[\]\(\)\{\}\]{0,3}[A-Za-z0-9_-]*[,:;\.\[\]\(\)\{\}\]{0,3}[A-Za-z0-9_-]*[,:;\.\[\]\(\)\{\}\]{0,3}[A-Za-z0-9_-]+$') | (R('(UBMOF|ZJU|SNU|MAF|MCF|Ir-MOF|JUC|FJI|UHM|MUV|BUT|ZJNU|Tb|she|CTH|pek|FDM|MODF|USF|NJFU|ZJU|CMOF|CTH|TPMOF|IZE|pbz|PNMOF|gea|MFM|UNLPF|PIZOF|soc|MOAAF|Y|Ho|CPO|JLU|aea|NOTT|NU|MMPF|UTSA|CPM|U[iI]O|MOF|IRMOF|T-MOF|NTU|MIL|HKUST|HNUST|LIC|PCN|ZIF|CPL|CALF|UMCM|DUT$)') +R('^[\-‐‑⁃‒–—―−－⁻]$')+ R('^[a-zA-Z0-9]+$'))
MOF_formula = R('^(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\dAs2O\d)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\dO\d\(OH\)\d\(RCO2\)\d\d?)(?![aA-zZ]+\-?\d?\d?\-ligands)(?!(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d\(CO2\)\d?\(?N?N?\)?)(?!^(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\($)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d?O?\(C?H?3?COO\)\d\s?\d?H?2?O?)(?!M1)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d\(μ(3|4)-OH?\)\d?\(R?CO2R?\)\]?\d?)(?!\[?Zn\d\(μ4-O\)\(O2CR?\)\d\])(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d?\((µ-OAc|µ-AcO|AcO|OAc)\)\d?\s?·?H2O)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d?\((µ-OAc|µ-AcO|AcO|OAc)\)\d?\s?\(H2O)(?!Me\dNH\d)(?!(\[?Zn\dO?\(O2C))(?!^M\(II\))(?!.*\(OOC\)\d$)(?!Co\(Tt\)\d?$)(?!\{?.*\(COO−?\)\d\}?$)(?!(M|Mn|Cu|Zn|Y|Co|Ni|Fe|Zr|Cd)\((I|II|i|ii|iii)\)$)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\d?O?\(R?CO2R?\)\d\]?$)(?!(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\dO?\+?.?.?.?$)(?!UiO-type$)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\(NO\d?\)\d?\]?.?.?.?.?.?.?.?$)(?!\[?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)\(ClO\d?\)\d?\]?.?.?.?.?.?.?.?$)(?!Cu\(NO\d?\).?.?.?.?.?.?.?.?$)(?!\[Zn3\(EBTC\)2\]∞$)(?!\{?Cu\d\(O2?CR\)\d\}?$)(?!HCl$)(?!Cu\(II\)$)(?!Zr6O8\(CH3COO\)26\+$)(?!Cu\(NO3\)2·3H2O$)(?!Zr6$)(?!Cu\(NO3\)2$)(\[?)(\{)?(\[)?(Mg|M|Ni|Zn|Mn|Cu|Zr|Me|CoCl|Co|Cd)(\s?)(L|\d|\(|\-)')
...
other_solvent = (
    ...
    I('n,n-dimethylacetamide') | I('n,n-dimethylformamide') | I('N,N-dimethylformamide') | I('n,n-DMF') | I('Na2SO4') | I('NaCl') | I('NaClO4') |
    ...
    I('xylene') | I('xylol') | I('N,N-diethylformamide') |
...
#Names we don't want the linker parser to pick up
linker_blacklist= Not(R('^C\d\d?H')) + Not(I('Nile') + I('Red')) + Not(R('^[tT]rifluoroacet(ate|ic)$') + Optional(R('^acid$'))) + Not(I('solid') + I('green')) + Not(I('HCl')) + Not(MOF_name) + Not(MOF_formula) + Not(I('diamond')) + Not(I('carbohydrate')) + Not(I('Soman')) + Not(R('^\d[aA-zZ]$')) + Not(R(u'^[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]+[0-9\)]+[Oo][Hh]$'))+Not(R(u'^[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]+[Oo][Ll]$'))+Not(R(u'^[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]+[0-9\)]+[Oo]$'))+Not(R(u'^[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]+[Oo][Nn][Ee]?$'))+Not(I(u'carbon(\W)dioxide'))+Not(I('sox'))+Not(I('so2'))+Not(I('h2s'))+Not(I('nox'))+Not(I('no'))+Not(I('no2'))+Not(I('h2'))+Not(I('n2'))+Not(I('h2'))+Not(I('co'))+Not(I(u'carbon\Wmonoxide'))+Not(I(u'ch4'))+Not(I(u'dcm'))+Not(I(u'dichloromethane'))+Not(I(u'ch2cl2'))+Not(I(u'Nh3'))+Not(I(u'ammonia'))+Not(I(u'methane'))+Not(I('co2'))+Not(I('o2'))+Not(element_name)+Not(element_symbol)+Not(I(u'DMF'))+Not(R(u'^[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]*[Dd]imethyl(\W){0,1}[Ff]ormamide?$'))+Not(I(u'DMSO'))+Not(R(u'^[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]*[Dd]imethyl(\W){0,1}[Ss]ul[phf]oxide?$'))+Not(I(u'DEF'))+Not(R(u'^[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]+[Dd]iethyl(\W){0,1}[Ff]ormamide?$'))+Not(I(u'DMA'))+Not(R(u'^[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]*[Dd]iethyl(\W){0,1}[Aa]cetamide?$'))+Not(I(u'acetone'))+Not(I(u'acetonitrile'))+Not(I(u'acetonitrile'))+Not(I(u'acetic(\W)acid'))+Not(I(u'formic(\W)acid'))+Not(I(u'NMP'))+Not(R(u'^[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]*[Mm]ethyl(\W){0,1}[a-zA-Z0-9\,\'-\′\-‐‑⁃‒–—―−－⁻]*[pP]yrrolidone?$'))+Not(R('^([A-K]|[a-k]|[m-z]|[M-Z])\d$')) + Not(R('^([A-K]|[a-k]|[m-z]|[M-Z])\d$')) + Not(I('Li')) + Not(I('amine'))+Not(R(u'^\[')) + Not(I('NH2')) + Not(I('magenta')) + Not(solvent_name) + Not(R('^[aA-zZ]$')) + Not(I('pyrene')) + Not(I('HCL')) + Not(R('^zeolites')) + Not(I('nuclear')) + Not(I('Oh')) + Not(R('^DUT\-\d\d?$')) + Not(I('carboxylic') + I('carbon')) + Not(I('blue') + Optional(I('line'))) + Not(I('benzoic') + Optional(I('acid'))) + Not(R('^phen(yl)?$')) + Not(R('^Me\dNH2')) + Not(R('^(ZJU|SNU|MAF|MCF|Ir-MOF|JUC|FJI|UHM|MUV|BUT|ZJNU|Tb|she|CTH|pek|FDM|MODF|USF|NJFU|ZJU|CMOF|CTH|TPMOF|IZE|pbz|PNMOF|gea|MFM|UNLPF|PIZOF|soc|MOAAF|Y|Ho|CPO|JLU|aea|NOTT|NU|UTSA|MMPF|CPM|U[iI]O|MOF|T-MOF|IRMOF|NTU|MIL|HKUST|HNUST|LIC|PCN|ZIF|CPL|CALF|UMCM|DUT)|(Zn˙˙˙Zn)|(acylamide)|(Cl\d)|(silica)|(acetic anhydride)|(1,4 di(x|o)(o|x)ane)|(\d\-P)|(C\d.?\d.?.?H.?.?.?.?.?)|(pseudo)|(Ag|Au|Br|Cd|Cl|Cu|Fe|Gd|Ge|Hg|Mg|Pb|Pd|Pt|Ru|Sb|Si|Sn|Ti|Xe|Zn|Zr|Ar|Vp)|(tetramethyl)|(nitrate)|(graphite)|acrolein|(BzOH)|fluoro|(Lewis acidic open copper)|(nile red)|(phosphoric acid)|(acetylene)|(BET)|(amide)|(^zeolite)|(COO)|(dry acetone)|(chloroform-exchanged)|(amino)|(trans-amide)'))

#options for the linker parser to pick up
linker_name_options = (
   cm | formula
)

linker_name = (Optional(include_prefix) + linker_blacklist + linker_name_options)('name').add_action(join).add_action(fix_whitespace)
